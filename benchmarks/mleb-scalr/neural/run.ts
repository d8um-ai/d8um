#!/usr/bin/env npx tsx
/**
 * MLEB-SCALR Benchmark — d8um Graph (Neural Search)
 *
 * Runs the isaacus/mleb-scalr benchmark
 ...
 *
 * Usage:
 *   npx tsx mleb-scalr/neural/run.ts           # query-only (uses existing index)
 *   npx tsx mleb-scalr/neural/run.ts --seed    # re-index corpus, then query
 */
...
const BUCKET_NAME = 'mleb-scalr-neural'
const TABLE_PREFIX = 'bench_mleb_neural_'
...
async function main() {
  ...
  console.log('╔══════════════════════════════════════════════════════════════╗')
  console.log('║  MLEB-SCALR — d8um Graph (Neural Search)                   ║')
  console.log('╚══════════════════════════════════════════════════════════════╝')
  console.log()
  ...
  const graph = createGraphBridge({
    ...
    llm,
    scope: { agentId: 'mleb-scalr-benchmark' },
  })
  ...
  console.log('Phase 2: Loading MLEB-SCALR from Vercel Blob...')
