#!/usr/bin/env npx tsx
/**
 * Contractual Clause Retrieval Benchmark — d8um Graph (Neural Search)
 *
 * Runs the isaacus/contractual-clause-retrieval benchmark
 ...
 *
 * Usage:
 *   npx tsx contractual-clause-retrieval/neural/run.ts           # query-only (uses existing index)
 *   npx tsx contractual-clause-retrieval/neural/run.ts --seed    # re-index corpus, then query
 */
...
const DATASET = 'contractual-clause-retrieval'
const BLOB_PREFIX = 'datasets/isaacus'
const BUCKET_NAME = 'contractual-clause-neural'
const TABLE_PREFIX = 'bench_contract_neural_'
...
async function main() {
  ...
  console.log('╔══════════════════════════════════════════════════════════════════╗')
  console.log('║  Contractual Clause Retrieval — d8um Graph (Neural Search)      ║')
  console.log('╚══════════════════════════════════════════════════════════════════╝')
  console.log()
  ...
  const graph = createGraphBridge({
    ...
    llm,
    scope: { agentId: 'contractual-clause-benchmark' },
  })
  ...
  console.log('Phase 2: Loading Contractual Clause Retrieval from Vercel Blob...')
  ...
  const result: BenchmarkResult = {
    benchmark: 'Contractual Clause Retrieval (isaacus)',
    dataset: DATASET,
    mode: 'neural',
    variant: 'graph',
