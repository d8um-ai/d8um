 ...
 *
 * Usage:
 *   npx tsx legal-rag-bench/neural/run.ts           # query-only (uses existing index)
 *   npx tsx legal-rag-bench/neural/run.ts --seed    # re-index corpus, then query
 */
...
const BUCKET_NAME = 'legal-rag-bench-neural'
const TABLE_PREFIX = 'bench_legalrag_neural_'
...
async function main() {
  ...
  console.log('╔══════════════════════════════════════════════════════════════╗')
  console.log('║  Legal RAG Bench — d8um Graph (Neural Search)               ║')
  console.log('╚══════════════════════════════════════════════════════════════╝')
  console.log()
  ...
  const graph = createGraphBridge({
    ...
    llm,
    scope: { agentId: 'legal-rag-bench-benchmark' },
  })
  ...
  if (!bucket || shouldSeed) {
    ...
    console.log(`Phase 3: Ingesting ${corpus.length} passages (with LLM triple extraction)...`)
  ...
  console.log(`Phase 4: Running ${qa.length} queries (mode: neural)...`)
